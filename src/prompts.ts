...
export async function askProjectQuestions(
  ...
  if (flags.silent) {
    return {
      ...
      standingOrders: existingAnswers?.standingOrders ?? [],
  ...
  // ── 5. Standing Orders ────────────────────────────────────────────────────
  log.section('Standing Orders');
  console.log(
    chalk.dim(
      '  Things you always tell Copilot — defined once, active forever.\n'
    )
  );

  const standingOrderPresets = [
    ...
    'When I ask you to document something, create or update the relevant .md file in the project',
    'Always suggest running the build command before git push',
    'When introducing new features, remind me to update the README',
    'When I fix a bug, suggest adding a regression test',
    'Always use the existing code patterns and conventions in this project — do not introduce new ones without asking',
    'When generating new files, follow the existing folder structure',
    'Never suggest installing a new dependency without checking if a similar library is already in use',
  ];

  const existingOrders = existingAnswers?.standingOrders ?? [];
  if (existingOrders.length > 0) {
    log.dim(`(${existingOrders.length} standing order(s) already set — adding new ones below)`);
    log.blank();
  }

  const { selectedOrders } = await prompt<{ selectedOrders: string[] }>({
    type: 'multiselect',
    name: 'selectedOrders',
    message: 'Select standing orders to always apply (space to toggle):',
    choices: standingOrderPresets,
  ...
  const standingOrders: string[] = [
    ...existingOrders,
    ...selectedOrders.filter((o) => !existingOrders.includes(o)),
  ];

  // Custom standing orders
  console.log(
    chalk.dim('\n  Add custom standing orders (blank to finish):')
  );
  let collectingOrders = true;
  while (collectingOrders) {
    const { order } = await prompt<{ order: string }>({
      type: 'input',
      name: 'order',
      message: `  Order ${standingOrders.length + 1} (blank to finish):`,
    });
    if (order.trim() === '') {
      collectingOrders = false;
    } else {
      standingOrders.push(order.trim());
    }
  }
