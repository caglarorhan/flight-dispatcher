...
export function extractPreservedData(existingContent: string): PreservedData {
  ...
  const behaviorSection = sections.find((s) => s.header === 'Copilot Behavior');
  ...
  const standingOrders = behaviorSection
  ...
  return { description, architectureRules, standingOrders, todos };
}
