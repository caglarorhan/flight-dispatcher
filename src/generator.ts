...
export function generate(
  ...
  const behaviorSection = buildCopilotBehavior(answers);
  if (behaviorSection) sections.push(behaviorSection);

  sections.push(buildDeployment(answers, detected));

  const hooksSection = buildGitHooks(hooksConfig);
...
function buildCopilotBehavior(answers: ProjectAnswers): string {
  if (!answers.standingOrders || answers.standingOrders.length === 0) return '';

  const lines: string[] = ['## Copilot Behavior', ''];
  for (const order of answers.standingOrders) {
    lines.push(`- ${order}`);
  }
  return lines.join('\n');
}

function buildGitHooks(hooksConfig?: HooksConfig): string {
  if (!hooksConfig || hooksConfig.hooks.length === 0) return '';

  const lines: string[] = ['## Git Hooks', ''];
  lines.push('The following automations run automatically via git hooks:');
  lines.push('');
  for (const hook of hooksConfig.hooks) {
    lines.push(`- \`${hook.trigger}\`: ${hook.label} — \`${hook.command}\``);
  }
  return lines.join('\n');
}

function buildTodos(answers: ProjectAnswers): string {  if (answers.todos.length === 0) return '';
